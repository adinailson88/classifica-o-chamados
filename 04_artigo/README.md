...
- As Tabelas 1 a 4 do corpo são floats LaTeX não divisíveis (`\begin{table}[!tbp]` com `tabularx`), e não mais `longtable`; a legenda e o rótulo (`\caption`/`\label{tab:...}`) ficam dentro do float, acima do `tabular`, e cada tabela usa ao menos uma coluna flexível do tipo X, com `\hsize` e `\linewidth` sincronizados; as colunas flexíveis limitam a tabela à largura declarada do `tabularx`, mantendo necessária a inspeção do PDF quanto a conteúdo não separável e legibilidade.
- **Rodada 9:** as Tabelas A1 a A3 do apêndice, que já cabiam cada uma inteira em uma única página mesmo como `longtable`, foram convertidas para floats não divisíveis no mesmo padrão das Tabelas 1 a 4, com a fonte subida de `\scriptsize` para `\footnotesize` (piso de legibilidade desta rodada) e numeração própria A1/A2/A3, independente da sequência 1–4 do corpo. A inspeção visual do PDF publicado após a PR #202 (21 páginas) confirmou que as Figuras 2, 4 e 5 permanecem legíveis nas dimensões da Rodada 8 (mantidas sem alteração) e que as Tabelas 1 a 4 seguem íntegras. O PDF final da Rodada 9 tem 22 páginas, dentro da faixa preferencial de 21 a 23, verificado por quatro renderizações reais via `workflow_dispatch` do `artigo_pdf.yml` na própria branch (sem tocar `main`).
...
- **Corrigido na Rodada 9:** o eixo Y da Figura 2 (trade-off de custo) arredondava os rótulos para uma casa decimal, fazendo marcas distintas (~0,80/0,82/0,85) aparecerem como "0,8" repetido. Corrigido no gerador canônico `src/gerar_figuras_canonicas.py` (parâmetro `casas` em `formatar_decimal()`, `casas=2` em `figura_custo()`); apenas `fig_tradeoff_custo.pdf`/`.png` foram regenerados, sob o mesmo `hash_corpus` canônico. O gerador legado `src/gerar_figura3_tradeoff_custo.py` não foi executado nem alterado.
