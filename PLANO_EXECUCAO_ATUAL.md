...
>
> **Estado geral:** execução canônica `1e476243` concluída; artigo auditado número a número contra os artefatos e sem resíduo de rodadas incompatíveis; o corpus congelado não tem data de abertura, de modo que a avaliação temporal é inexecutável e o alcance prospectivo está declarado como condicional; título, resumos, Introdução, Referencial e Método reposicionados em torno do protocolo auditável; a Seção 4 foi reestruturada em cinco subseções hierarquizadas, sem vencedor absoluto declarado; a Discussão foi reestruturada em quatro subseções (5.1 a 5.4) sem redundância remanescente com a Seção 4, e o corpo científico está em 8.915 palavras (rotina única de contagem), dentro da faixa-meta de 8.850 a 9.000; as Figuras 2, 4 e 5 tiveram a largura reduzida em 20% e as Tabelas 1 a 4 passaram a floats LaTeX não divisíveis, com legenda e rótulo dentro do float (`\caption`/`\label`), colunas X ponderadas com `\hsize` e `\linewidth` sincronizados, e a lista de referências caiu de 49 para 45 entradas após a remoção de quatro já órfãs; a PR #202 recebeu duas rodadas de auditoria independente. Na Rodada 9, a auditoria visual do PDF publicado confirmou legibilidade das Figuras 2, 4 e 5 nas dimensões da Rodada 8 (mantidas) e integridade das Tabelas 1 a 4; as Tabelas A1 a A3 do apêndice, que já cabiam cada uma numa única página mesmo em `longtable`, foram convertidas para floats não divisíveis com fonte subida de `\scriptsize` para `\footnotesize` e numeração própria A1/A2/A3; o PDF final da rodada tem 22 páginas, dentro da faixa 21–23. Em microcorreção posterior na mesma rodada, o arredondamento do eixo Y da Figura 2 (marcas repetidas por causa de uma casa decimal) foi corrigido em `src/gerar_figuras_canonicas.py`, com apenas `fig_tradeoff_custo` regenerado sob o mesmo hash canônico; o gerador legado `gerar_figura3_tradeoff_custo.py` não foi executado nem alterado. A PR #203 (Rodada 9) foi mesclada em `main` (`69052405`) e o PDF foi regenerado automaticamente pelo workflow pós-merge (`7338214f`). A Rodada 10 é uma auditoria final de submissão, sem novos números nem reestruturação, registrada em `docs/AUDITORIA_FINAL_SUBMISSAO.md`. A Rodada 11 integrou ao artigo o resultado confirmatório do ensemble da Fase 2C (PR #209, execução científica preservada em `docs/dados/ensemble/fase2c/`): votação majoritária, votação suave ponderada e stacking, comparados ao LinearSVC na mesma capacidade `K_f` sobre 593 casos de `Y=1`, não superaram o baseline; a nova Tabela S17 do suplemento é gerada programaticamente a partir do manifesto congelado, sem retreino de nenhum modelo. O corpo científico passou de 8.915 para 8.999 palavras, ainda dentro da faixa-meta.
>
...
Atualizado em 14/08/2026, no fuso America/Bahia.
