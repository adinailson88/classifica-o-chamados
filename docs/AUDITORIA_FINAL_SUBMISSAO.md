...
- **Material suplementar**: a Subseção 4.3 cita explicitamente "Tabela S16"; as demais remissões ao longo do corpo usam a expressão genérica "material suplementar", sem numeração — consistente com o uso já estabelecido nas rodadas anteriores. Os CSVs numerados presentes em `04_artigo/figuras/` vão de S1 a S16, **com uma lacuna: não existe `tabela_S4_holdout_vs_kfold.csv`**, embora o gerador `src/comparacao_holdout_kfold.py` produza esse nome de arquivo. S4 não é citado por número em nenhum ponto do corpo do artigo. Registrado como item de decisão do autor na Seção 9; nenhuma renumeração foi aplicada por não ser objetiva nem de baixo risco sem confirmação de que o artefato foi de fato descontinuado.
...
Nenhuma alteração tocou `docs/dados/*.json`, hashes, partições, categorias, denominadores ou conclusões científicas.
...
- **Lacuna de numeração no material suplementar (S4)**: o gerador `src/comparacao_holdout_kfold.py` produz `tabela_S4_holdout_vs_kfold.csv`, ausente de `04_artigo/figuras/`, e a numeração salta de S3 para S5. Como S4 não é citado por número em nenhum ponto do corpo, a lacuna não quebra nenhuma remissão do artigo, mas o autor deve decidir se regenera o artefato, renumera o suplemento sem lacuna, ou documenta a descontinuação como intencional.
