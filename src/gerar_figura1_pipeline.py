...
def gerar(saida: Path = SAIDA) -> list[Path]:
    ...
    for i, texto in enumerate(ETAPAS):
        x, y = _posicao(i)
        cor = COR_DESTAQUE if i == len(ETAPAS) - 1 else COR_CAIXA
    ...
    # Retroalimentacao: da etapa 8 (validacao humana) de volta para a etapa 4
    # (treino). O trajeto contorna as caixas pela direita.
    x8, y8 = _posicao(len(ETAPAS) - 1)
    x4, y4 = _posicao(3)
    margem = x8 + 1.32
    volta = y8 - 0.30
    ax.plot([x8 + 0.5, x8 + 0.5], [y8, volta], color=COR["vermelho"], linewidth=1.1)
    ax.plot([x8 + 0.5, margem], [volta, volta], color=COR["vermelho"], linewidth=1.1)
    ax.plot([margem, margem], [volta, y4 + 0.45], color=COR["vermelho"], linewidth=1.1)
