#!/usr/bin/env python3
"""Tabelas suplementares S6 a S16, a partir da rodada canonica.
...
citar seus valores em prosa:
    ...
    S6   dispersao das predicoes: entropia normalizada e Jensen-Shannon
    S7   curva ABC global, F1 macro por classe e por modelo
    S8   tarefa de tipo de manutencao
    S9   curva ABC interna a cada tipo, para o LinearSVC
    S10  efeito da camada de regras de periodicidade
    S15  calibracao completa dos sete modelos, resumida no corpo (Tabela 4)
    S16  Fase 2C: LinearSVC vs combinacoes (votacao majoritaria, votacao
         suave, stacking), agregado e por outer fold.

Le apenas artefatos com `hash_corpus` da rodada canonica e aborta se houver
divergencia. S16 e excecao: sua fonte exclusiva e o manifesto confirmatorio
...
def s6_dispersao(historica: dict[str, Any]) -> Path:
    linhas = []
    ...
    return escrever("tabela_S6_dispersao_predicoes.csv",
                    ["modelo", "categorias_previstas", "entropia_normalizada",
                     "js_contra_o_historico"], linhas)


def s7_abc_global(recortes: dict[str, Any]) -> Path:
    linhas = []
    for m in recortes["modelos"]:
        for c in m["curva_abc"]:
            linhas.append([NOME.get(m["modelo"], m["modelo"]), c["classe"],
                           ...
                           c["macro_f1"]])
    return escrever("tabela_S7_curva_abc_global.csv",
                    ["modelo", "classe", "categorias", "chamados",
                     "proporcao_do_volume", "acuracia", "macro_f1"], linhas)


def s8_tarefa_tipo(recortes: dict[str, Any]) -> Path:
    linhas = []
    ...
    return escrever("tabela_S8_tarefa_tipo.csv",
                    ["modelo", "acuracia", "macro_f1", "f1_preventiva",
                     "f1_corretiva", "f1_nao_manutencao"], linhas)


def s9_abc_por_tipo(recortes: dict[str, Any]) -> Path:
    ...
    for tipo, classes in modelo["curva_abc_por_tipo"].items():
        for c in classes:
            linhas.append([tipo, c["classe"], c["categorias"], c["chamados"],
                           c["proporcao_do_volume"], c["acuracia"],
                           c["macro_f1"]])
    return escrever("tabela_S9_curva_abc_por_tipo.csv",
                    ["tipo", "classe", "categorias", "chamados",
                     "proporcao_do_volume_do_tipo", "acuracia", "macro_f1"],
                    linhas)


def s10_regras(regras: dict[str, Any]) -> Path:
    ...
    return escrever("tabela_S10_regras_versus_modelos.csv",
...
def s11_inferencia_agrupada(agrupada: dict[str, Any]) -> Path:
    ...
    return escrever("tabela_S11_inferencia_agrupada.csv",
...
def s12_classes_raras(sensibilidade: dict[str, Any]) -> Path:
    ...
    return escrever("tabela_S12_classes_raras.csv",
                    ["modelo", "acuracia", "macro_f1_41_avaliadas",
                     "macro_f1_50_taxonomia", "macro_f1_14_familias"], linhas)


def s13_utilidade(utilidade: dict[str, Any]) -> Path:
    ...
    return escrever("tabela_S13_utilidade_reclassificacao.csv",
...
def s14_pressupostos(inferencia: dict[str, Any]) -> Path:
    ...
    return escrever("tabela_S14_pressupostos.csv",
...
def s15_calibracao_completa(calibracao: dict[str, Any]) -> Path:
    ...
    return escrever("tabela_S15_calibracao_completa.csv",
...
def s16_ensemble_confirmatorio() -> Path:
    ...
    return escrever(
        "tabela_S16_ensemble_confirmatorio.csv",
...
def main() -> int:
    ...
    for caminho in (s6_dispersao(historica), s7_abc_global(recortes),
                    s8_tarefa_tipo(recortes), s9_abc_por_tipo(recortes),
                    s10_regras(regras), s11_inferencia_agrupada(agrupada),
                    s12_classes_raras(sensibilidade), s13_utilidade(utilidade),
                    s14_pressupostos(inferencia),
                    s15_calibracao_completa(calibracao)):
        print(f"  {caminho.name}")

    caminho_s16 = s16_ensemble_confirmatorio()
    print(f"  {caminho_s16.name} (fonte: fase2c_execucao_cientifica_1_manifest.json, sem hash_corpus)")
